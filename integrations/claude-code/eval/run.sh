...
if command -v jq >/dev/null; then
  jq -e 'select(.tool_use_result.commandName=="aep:planning")
         | select(.tool_use_result.success==true)' "$WORK/result.jsonl" >/dev/null 2>&1 && R=0
else
  grep -q '"commandName":"aep:planning"' "$WORK/result.jsonl" && R=0
fi
...
check "hermetic: only aep loaded (saw: ${PLUGINS:-unknown})" "$R"
