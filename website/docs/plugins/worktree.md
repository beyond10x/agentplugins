...
`worktree` binary. This marketplace carries no copy: its Claude Code entry pins the worktree
repository by release tag and full commit, and `agentplugins-check remote` refuses a pin that is
not the newest release or whose manifests declare another version.

```text
/plugin install worktree@b10x
```

Codex reads the worktree repository's own marketplace:
