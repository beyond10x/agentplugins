...
- a `setup` skill that drives the `b10x` binary: it reads both hosts, the binaries on `PATH` and
...
- a `guide` routing skill for selecting `aep-plan`, `aep-drive`, `ess`, `worktree` or
  `connectors`;
