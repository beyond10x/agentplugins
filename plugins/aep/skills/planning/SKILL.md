...
**Skill version 0.14.2** — the version in `.claude-plugin/plugin.json`.
...
|---|---|
| `aep:plan-critic-acceptance` | could anybody ever tell whether these are done? |
| `aep:plan-critic-design` | is this set the right shape — coupling, cycles, a split abstraction? |
| `aep:plan-critic-scope` | is everything the parent promised claimed, and nothing else? |
