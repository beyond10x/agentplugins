...
1. `aep plan artifact list --format json` — what already exists. Stories may already be derived from
...
3. Anything the epic relates to. `aep plan artifact graph` shows the edges; follow the ones that
   change what "covered" means.
4. `aep plan artifact kinds` and `aep plan artifact lifecycle story` if you have not read them in
...
the ownership and the cardinality, refused by `ess specify validate` when the target does not exist, the
...
```console
$ aep plan artifact new decision-blocker workspace-team-ownership \
...
`aep plan artifact relations` for the edge, and `aep plan artifact lifecycle decision-blocker` for the ladder
the blocker lands on and the move that clears it. `aep plan artifact kinds` names the blocker *family*,
...
1. **Never move an artifact out of its initial status.** You do not run `aep plan artifact move`, for any
...
3. **Never edit a planning-store file directly.** Relations are set with `--relate` at creation or
   `aep plan artifact relate`; bodies use `aep plan artifact body`; status uses `aep plan artifact
...
5. **Finish with `aep plan artifact validate`.** Always, even when you believe nothing can be wrong.
...
4. The full output of `aep plan artifact validate`, verbatim, and its exit status.
