...
`aep artifact body <story-id> --from <path|->`: the context, and **one acceptance statement**
...
1. **Never move an artifact out of `draft`.** You do not run `aep artifact move`, for any
...
4. **Finish with `aep artifact validate`.** Always, even when you believe nothing can be wrong.
